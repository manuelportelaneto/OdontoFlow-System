...
const flattenKeys = (obj, prefix = []) => {
  return !(0, import_isObject.default)(obj) ? { [prefix.join(".")]: obj } : (0, import_reduce.default)(
    obj,
    (cum, next, key) => (0, import_merge.default)(cum, flattenKeys(next, [...prefix, key])),
    {}
  );
};
...
const compareItems = (obj1, obj2, keys, disableDotNotation = false) => {
  ...
  for (const key of keys) {
    if (!disableDotNotation) {
      if (!(0, import_isEqual.default)((0, import_get.default)(obj1.json, key), (0, import_get.default)(obj2.json, key))) {
        result = false;
        break;
      }
    } else {
      if (!(0, import_isEqual.default)(obj1.json[key], obj2.json[key])) {
        result = false;
        break;
      }
    }
...
function updateDisplayOptions(displayOptions, properties) {
  return properties.map((nodeProperty) => {
    return {
      ...nodeProperty,
      displayOptions: (0, import_merge.default)({}, nodeProperty.displayOptions, displayOptions)
    };
  });
}
...
function isFalsy(value) {
  if ((0, import_isNull.default)(value)) return true;
...
const parseStringAndCompareToObject = (str, arr) => {
  try {
    ...
    return (0, import_isEqual.default)(parsedArray, arr);
  } catch (error) {
    return false;
  }
};
const fuzzyCompare = (useFuzzyCompare, compareVersion = 1) => {
  if (!useFuzzyCompare) {
    return (item1, item2) => (0, import_isEqual.default)(item1, item2);
  }
  return (item1, item2) => {
    if (!(0, import_isNull.default)(item1) && !(0, import_isNull.default)(item2) && typeof item1 === typeof item2) {
      return (0, import_isEqual.default)(item1, item2);
    }
    if (compareVersion >= 2) {
      if ((0, import_isNull.default)(item1) && ((0, import_isNull.default)(item2) || item2 === 0 || item2 === "0")) {
        return true;
      }
      if ((0, import_isNull.default)(item2) && ((0, import_isNull.default)(item1) || item1 === 0 || item1 === "0")) {
        return true;
      }
    }
    ...
    if (!(0, import_isNull.default)(item1) && typeof item1 === "object" && typeof item2 === "string") {
      return parseStringAndCompareToObject(item2, item1);
    }
    if (!(0, import_isNull.default)(item2) && typeof item1 === "string" && typeof item2 === "object") {
